...
expected_timeseries = torch.cat(
    (torch.add(past_error.squeeze(), Y[:past_horizon].to(device)), forecast.squeeze()), dim=0
).detach().cpu()
...
prosper_nn.utils.visualization.plot_heatmap(
...
with torch.no_grad():
    hcnn_model.eval()
    output_forecast = hcnn_model(Y_batches[-1].cpu())
    ...
    for batch_index in range(0, Y_batches.shape[0]):
        Y_batch = Y_batches[batch_index]
        model_output = hcnn_model(Y_batch.cpu())
